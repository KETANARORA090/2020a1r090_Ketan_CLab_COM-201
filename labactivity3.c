...
#include<stdio.h>     // main header file (printf and scanf)
...
void push(struct Stack *stack, char element){       //function parameters
    if(stack->top == stack->size - 1){      //if stack top is equal to stack size - 1 (isFull function)
...
char pop(struct Stack *stack){              //function parameters
...
int isBalanced(struct Stack *stack, char *expression){ //function parameters
    for (int i = 0; expression[i] != '\0'; i++){        // for loop will run till the end of the string
        ...
        if(isEmpty(stack)){                        //to check stack is empty(if empty means balanced) 
         return 1;                                       
    }
    else{
        return 0;                               // not empty  means expression is not balanced 
    }
}


int main(){
    struct Stack stackA;            // declare stackA
    char expression[50];           //for expression (array)
